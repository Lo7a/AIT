...
- דוחות נוספים בתיקייה (`bugs/ssrf-redirect-bypasses-host-allowlist.md`, `features/*`, `reviews/*`) לא נכללו בסבב הזה ואין עליהם פסיקה כאן.
- `features/shared-prompt-fencing-utility.md` נסגר דה-פקטו כתוצר לוואי של תיקון גדר הפרומפטים: יש עכשיו פונקציה משותפת אחת.
- אימות: `npx vitest run` - 538 בדיקות עוברות, `npx tsc --noEmit` נקי.
