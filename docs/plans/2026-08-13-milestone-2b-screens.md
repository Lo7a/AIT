...
**חסמי-deploy (מסקירת אבטחה על משימה 8 — לא חוסמים 2ב המקומית, חוסמים כל חשיפה ציבורית):** (1) `/api/diagnose` הוא endpoint שמפעיל פעולה בתשלום — אסור לחשוף ציבורית בלי אימות + rate-limit (Auth מתוכנן ממילא אחרי ה-MVP); (2) הגנת ה-SSRF במסלול ה-URL היא מבוססת-שם בלבד — לפני deploy נדרשת חסימה אחרי DNS resolution (מניעת rebinding); (3) "הסריקה שורדת ניתוק" אומת על שרת Node חי — ב-Vercel serverless יש לעטוף ב-after() של Next 15 כדי שהסריקה תשלים אחרי סגירת התגובה; (4) ה-guard נגד ירי-כפול של סריקה הוא per-tab (מודול בדפדפן) — שני טאבים או reload קשיח יכולים לירות כפול; לפני deploy נדרש dedup צד-שרת לפי יעד מנורמל (in-flight map או 409).

**הנחיית מייסד (2026-08-14, מחייבת לכל 2ב והלאה):** אפס סימני-כתיבה של AI במוצר — בלי קו מפריד ארוך (מקף רגיל בלבד), בלי אמוג'י ב-UI, קופי עברי אנושי; עיצוב ייחודי ולא AI-גנרי. יושם: סוויפ מלא של מחרוזות המשתמש (7a272d1) + שפה עיצובית עריכתית שנגזרה מסקילים של להב — Frank Ruhl Libre לכותרות + Assistant לגוף, קנבס שנהב, CTA דיו, פסטלים סמנטיים בלבד, שורות קו-שיער, אנימציות כניסה מדורגות (4863c7d). סוקר העיצוב פסק: "נקרא כעריכתי-מעוצב, לא תבנית".
...
> **As-built (f2b650c):** בוצע עם הקשחה מעל התוכנית: query נבדק ב-typeof string (מספר/אובייקט → 400 בלי לקרוא לחיפוש, במקום המרה שקטה), 7 בדיקות במקום 5. נבדק בטיחותית: מפתח ה-API נוסע ב-header בלבד ולא יכול לדלוף בהודעות שגיאה (readErrorBody מחזיר רק גוף תגובה). הערה למסכים (משימות 9–10): שגיאות 502 עוברות verbatim ללקוח — גוף 403 של גוגל מכיל מספר פרויקט GCP; ה-UI יציג fallback עברי גנרי ל-502 וילוגג את המקור בצד שרת.
...
> **As-built (ec506c7 + d2d54e4):** בוצע + הקשחה מסקירת אבטחה: guard SSRF מבוסס-שם (לופבק/פרטי/link-local, כולל IPv6 עם טיפול בסוגריים ש-URL.hostname שומר), סניטציית שגיאות (DiagnoseFailed עובר בעברית; כל השאר → גנרית + לוג שרת), city לא-מחרוזת נדחה, name נחתך, ו-Promise.resolve() נגד זריקה סינכרונית. הסוקר אימת אמפירית: ניתוק באמצע זרם — ה-runner רץ עד הסוף, אפס unhandled rejections; ה-catch על enqueue וה-catch על close שניהם קוד חי (שניהם זורקים בפועל אחרי cancel); הזרקת NDJSON בלתי-אפשרית (stringify מבריח \n). חסמי-deploy נרשמו למעלה. 230 בדיקות, 17 בקובץ.
...
> **As-built (2d1b3c5 + 7a272d1 + 4863c7d + סבב תיקוני סקירה):** נבנה כמתוכנן ואז עבר את סוויפ הכתיבה ושדרוג השפה העיצובית. סקירה משולשת (התנהגות/זרם/עיצוב, אומתה אמפירית מול שרת חי בלי אף קריאת API בתשלום) אישרה את המשפך כולו: פיצול 502/400, דאבל-קליק בטוח (קריאת חיפוש אחת, POST אבחון אחד), שמות עברית עוברים שלמים ב-URL, force-dynamic פעיל. תיקונים שאומצו: זיהוי URL תומך דומיין עברי (אתר.ישראל), אימייל לא מסווג כ-URL, "http://" לבדו נדחה, סניטציית 502 בצד שרת, timeout לחיפוש, העברת עיר ליעד האבחון, aria-labels ו-role=alert.
...
> **As-built (4863c7d + סבב תיקוני סקירה):** נבנה בשפה העיצובית הסופית: שורות מופרדות קו-שיער (לא כרטיסים), אינדיקטורים SVG (בלי אמוג'י), כניסות מדורגות, סריף עברי לכותרת. ה-guard נגד ירי-כפול: Set ברמת המודול לפי JSON של היעד + ref לפי מפתח (לא בוליאני — ניווט searchParams-בלבד לא נחסם בטעות). באג חשוב שנתפס בסקירה ותוקן: אירועי done/error חייבים לשחרר את הנעילה גם אחרי unmount (לפני בדיקת cancelled), אחרת back+forward באמצע סריקה נועל את היעד לכל הסשן. שחרור לא מתבצע על שבירת זרם באמצע (הסריקה בצד שרת כנראה שולמה — כפילות אסורה); ההודעה מפנה לרשימת העמוד הראשי. aria-live/role=status/aria-busy לקוראי מסך; טוקן משני הוכהה ל-#6F6E6A (WCAG AA); מיקודי מקלדת דיו (בלי הטבעת הכחולה של כרום).
...
> **As-built (78398a0):** נבנה לפי הנחיית סוקר העיצוב: פתיח "פסק דין עריכתי" — ציון ענק בסריף טבלאי לצד כותרת הנרטיב וסיכום כ-deck עיתונאי — במקום דשבורד; ממדים כשורות קו-שיער עם תגי-סטטוס פסטליים; שקיפות בכל חוק דרך details נטיבי (בלי JS); באנר no_gbp; מד שלמות עם הצעד הבא; CTA ראיון/Roadmap מנוטרלים (אבני דרך 3-4). אומת חי מול אבחון אמיתי מה-DB בזמן הפיתוח.
...
### משימה 12: שער יציאה 2ב — הרצה חיה מלאה בדפדפן ✅ עובר

> **As-built (25ab65b):** השער הורץ בדפדפן Chrome אמיתי (chrome-devtools MCP) מול שרת production בפורט 3210. שלוש הריצות עברו: אופטיקה בק 73/100 (זהה ל-2א; הזרמה פרוגרסיבית הוכחה בחותמות זמן), ברכת רחל 84/100 (סטייה מ-77 מוסברת: מדגם הביקורות היומי בלי תמות בעיה; "אין מידע" ולא 0), lavangroup 63/100 (סטייה מ-71: PSI חי 40/100; $0.000 Places מוכיח מסלול אתר-בלבד; באנר no_gbp בולט). עמידות: רענון באמצע — report_ready תוך 28.1 שניות, מופיע ברשימה עם קישור עובד; תופעת לוואי מתועדת: הרענון ירה סריקה כפולה (ה-guard מתאפס ב-reload קשיח) — מחזק את חסם-ה-deploy של dedup צד-שרת. ToS: אפס דליפות בכל 5 הסריקות הטריות וה-HTML. היגיינת כתיבה: אפס קווים ארוכים בדוחות טריים. עלות השער: $0.24.
