...
- [x] Commit: `docs(4-9): milestone 4 exit gate`

**As-built (15.8):** השער רץ חי במלואו - 6 עוברות, 2 נכשלות בכנות: (4) confidence high לרודמאפ בלי ראיון - הפישוט של משימה 3 השמיט את דרישת ה-credit מהספק, תיקון בהמשך; (7) staleness אמיתי בחומרה גבוהה - הציונים השמורים של אופטיקה בק (73, process אין-מידע) מול הטריים (58, process 0, topGaps אחרים), הנרטיב מסביר פערים שכבר לא מובילים, וה-Business Map במסך 5 קורא מהשמורים בעוד הפריטים נגזרים מהטריים. הוקלט כחוסם פריסה. הפתעה חיובית: כל השער רץ עם Gemini ב-429 מלא (מכסה יומית) - כל הנימוקים fallback, אפס קריסות. 35 מספרים מוצגים, אפס לא-ממקור. build נקי 15 נתיבים.
