...
**הערת as-built 26.8 (עיצוב המסך, קומיטים 2077ad6 עד 39256b4):** המסך עבר מצ'אט אחד ארוך לשלושה פאנלים בגובה המסך, מימין לשמאל: רשימת השאלות הממוספרת, השאלה הנוכחית, והשיחה. הרשימה: interviewPlan ב-questions.ts מדמה את pickNextQuestion עצמה ולכן המספור אומר את האמת על הסדר; לכל שאלה בבנק label קצר; שאלה שנענתה ניתנת ללחיצה ונפתחת לעריכה עם התשובה הקודמת מסומנת (revisit ב-reducer - השומר שם, לא רק ב-disabled; קדימה רק בתשובה או בדלג); התשובה השמורה מוצגת מתחת לכל תווית (answersOf, אותן הודעות שהעריכה קוראת). השאלה הנוכחית בפאנל נמתח עם שורת הפעולות נעוצה למטה; השיחה בפאנל משלה עם תיבת הכתיבה בתחתיתה. הדף לא נגלל: .main-col:has(> .repC.iv) בגובה החלון וכל פאנל גולל בתוך עצמו (קודם כל הודעה הזיזה את המסך מתחת לעכבר). מתחת ל-1360 השיחה יורדת מתחת לשאלה והרשימה לאורך שתיהן; מתחת ל-1100 עמודה אחת בגלילת דף. הפנקס עבר לרצועת הניווט (CompletenessCard compact) ושורת העובדות בסרגל היא BusinessFacts, אותה אחת כמו בדוח. QuestionView ו-PlanItem ב-run-interview.ts, plan נשלח ב-snapshot ובכל תור. 1378 בדיקות.
...
**הערת as-built (בוצע, שער עבר):** ראיון השער רץ על אופטיקה בק (d04eaa39) ישירות מול המנוע אחרי הנחיית המייסד לוותר על בדיקות דפדפן בשלב הזה (עשן ה-UI בדפדפן בוצע קודם על כפיל VFORCE, כולל אימות הפוקוס). שלמות 30 אל 90 (יעד 60), 5 מונחות + חופשי, אפס נפילות LLM, אינווריאנט resume בכל תור, אפס המצאות על 6 שדות, תקרת 12 מאומתת. שני ממצאי שער אמיתיים נתפסו ונסגרו בו-ביום: תווים אסורים בתשובות LLM (normalizeTypography, aa67812) וסבב הסקירה הסופית שכיסה את חוב משימה 6 (InterviewError מתויג במקום יוריסטיקת עברית, תקרת גוף 4000, הלקוח מכבד סטטוס snapshot, עיגון חוזה NOT_ACTIVE, ועוד - 4d130c6). 379 בדיקות, tsc ו-build נקיים (12 ראוטים). מסמך מלא: docs/milestone-3-gate.md.
