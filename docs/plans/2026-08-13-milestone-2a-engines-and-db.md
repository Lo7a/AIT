...
> **הערת as-built — סבב סקירה שני (לפני משימה 12, שהקוד הקורא הופך את הבאג לבר-הגעה):**
> 1. **שער לחוסר-מזהה (מנע שחיתות חוצה-עסקים בשקט):** `createDiagnosisForBusiness` עם לא `placeId` ולא `website` היה מריץ `findFirst({ where: { website: undefined } })` — Prisma מוחק מפתח עם ערך `undefined`, אז ה-`where` נהיה ריק ומחזיר עסק **שרירותי כלשהו** מהטבלה, ומצמיד לו אבחון של מישהו אחר. תוקן: הענף השני פוצל ל-`else if (input.website)` (מסלול אתר-בלבד כרגיל) ו-`else` חדש שזורק `Error("createDiagnosisForBusiness: נדרש placeId או website")` — בלי לגעת ב-DB בכלל.
> 2. **`transitionDiagnosis` הפך race-safe:** `prisma.diagnosis.update` הרגיל (ללא תנאי) הוחלף ב-`updateMany` מותנה גם על ה-`status` שנקרא (`where: { id, status: current.status }`) — שתי קריאות מקבילות על אותו אבחון לא יכולות שתיהן לעבור; קריאה שמאבדת את המרוץ מקבלת `count: 0` וזורקת שגיאה ייעודית ("מעבר סטטוס נכשל — הסטטוס השתנה במקביל"). זה סוגר חלון TOCTOU בין ה-`findUniqueOrThrow`/`assertTransition` הראשוני לבין הכתיבה.
> 3. **`saveScanResult` הפך אטומי:** שתי הכתיבות (`scan.create` + `businessModelRow.upsert`) עטופות ב-`prisma.$transaction([...])` (צורת מערך — שתי הפרומיסים כבר "בנויים" אבל Prisma שולח את שתי השאילתות יחד בטרנזקציה אחת בסדר שנשמר) — כדי שכישלון בכתיבה השנייה לא ישאיר `scan` יתום בלי `business_model` תואם.
> 4. **מבחנים הורחבו מ-6 ל-9:** `fakePrisma` קיבלה `create`/`updateMany` (בלי יותר reassignment ל-`(prisma as Record<string, unknown>)` בכל מבחן), נוסף מבחן race על `updateMany` שמחזיר `{count: 0}`, נוסף מבחן על חוסר-מזהה כפול (`business.findFirst` לא נקרא בכלל), נוסף `describe("saveScanResult", ...)` שמוודא גם את שדות ה-`scan.create` (findings/scores/narrative/llmCost/apiCost/durationMs) וגם ש-`credits` מופיע בשני ענפי ה-`upsert` (עם fake `$transaction: vi.fn((ops) => Promise.all(ops))`), ומבחן ה-upsert-לפי-placeId הודק ל-`expect.objectContaining({ where: { placeId: "p1" } })`. המקור המחייב: `src/server/diagnosis-repo.ts`, `tests/diagnosis-repo.test.ts`.
...
> **הערת as-built (סבב סקירה שני — תיקוני חובה לפני מיזוג + תיקונים זולים):**
> 7. **אימות ארגומנטים לפני כל קריאת API חי (חוסם מיזוג):** הגרסה הראשונה של `parseArgs` השאירה `--url` בלי ערך (או `--url ""`) בתוך ה-`query` בשקט — מה שגרר חיפוש Places חי על המחרוזת הלא-תקינה "--url". `parseArgs` שוכתב: כל דגל (`--pick`/`--url`, גם בצורת `=`) שנראה ("נראה" = `--pick`/`--url` מופיע ב-argv) אבל בלי ערך תקין מוחזר כ-`{ error: string }` **לפני** שהפונקציה מחזירה `query`/`pick`/`url` לשימוש — `main()` בודק `parsed.error` ויוצא בקוד 1 מיד, לפני כל `pickCandidate`/`normalizeSiteUrl`. גם `--pick=abc` (לא מספר) וגם `--pick 0`/`--pick -1` (לא שלם חיובי) נדחים כאן — הרחבה מעבר לבדיקת ה-`Number.isInteger` הבודדת של `src/cli.ts` (שם אין בדיקת `<= 0`). מכוסה ב-`tests/cli-diagnose-args.test.ts` (7 מבחנים, אופליין לגמרי — פונקציה טהורה).
> 8. **השלמת `website` לשורת ה-Business (מסלול Places):** אחרי `runScan` מוצלח (כשאין `siteUrl`, כלומר מסלול חיפוש-עסק ולא `--url`) ואחרי המעבר ל-`scanned`, אם `scan.business.website` מוגדר — `prisma.business.update({ where: { id: created.businessId }, data: { website: scan.business.website } })`. קודם לתיקון הזה שורת ה-Business נשארה עם `website: null` לצמיתות גם כשה-JSONB של הסריקה ידע את הכתובת (אומת מול ה-DB החי). `city` **לא** מתעדכן — ל-`ScanFindings.business` אין שדה כתובת/עיר היום; זה חוב טכני נרשם כאן ולא נפתר במשימה זו (מקור עתידי אפשרי: `PlaceDetails`/Places API מחזיר `formatted_address` שלא נשמר כרגע ב-`business.ts`).
> 9. **שגיאת הסריקה המקורית לא נבלעת ב-revert:** ב-`catch` שמנסה להחזיר את הסטטוס ל-`created` אחרי כישלון סריקה, קריאת `transitionDiagnosis(..., "created")` עטופה ב-`try/catch` נפרד; אם ה-revert עצמו נכשל (למשל race על הסטטוס) — נרשם `console.error` ייעודי, אבל השגיאה שממשיכה להיזרק היא תמיד שגיאת הסריקה המקורית (`err`), לא שגיאת ה-revert. בלי זה, revert שנכשל היה מחליף את סיבת הכישלון האמיתית בהודעת state-machine לא קשורה.
> 10. **אזהרה כש-`--url` מגיע יחד עם שם עסק/`--pick`:** מסלול `--url` תמיד גבר (ללא שינוי בהתנהגות) — אבל עכשיו, אם גם `query` וגם `pick` הוגדרו לצד `--url`, מודפסת שורת אזהרה חד-שורתית שהם מתעלמים, כדי שזה לא יקרה בשקט.
> 11. **קוד יציאה עקבי למועמדים מרובים:** מסלול חיפוש-עסק (`!url`) עבר מ-`process.exit(1)` קבוע ל-`process.exit(picked.ambiguous ? 0 : 1)` — מיושר עם `src/cli.ts`, ששם "כמה מועמדים, אין `--pick`" הוא לא שגיאה (יציאה 0) ורק "לא נמצא"/"pick מחוץ לטווח" הם שגיאה (יציאה 1).
> 12. **שורת הפער החיובי מותנית ב-`overall != null`:** בגרסה הקודמת `topGaps.length === 0` הדפיס "לא נמצאו פערים מהותיים... בסיס דיגיטלי חזק" גם כש-`overall` הוא `null` (כלומר: אין בכלל מידע לאף ממד, לא ש"הכול תקין") — סתירה סמויה מול שורת "ציון כולל: אין מספיק מידע" שכבר הודפסה למעלה. עכשיו הענף השני הוא `else if (score.overall != null)`; כש-`overall` הוא `null` וגם `topGaps` ריק — לא מודפסת אף שורה בסקציה הזו. מבחן נעילה שלישי נוסף ל-`tests/cli-format.test.ts` (`overall: null` + `topGaps: []` → לא מכיל "בסיס דיגיטלי חזק"/"לא נמצאו פערים מהותיים").
> 13. **מבחן פערים ריקים ננעל על הטקסט עצמו:** `tests/cli-format.test.ts` הוסיף `expect(text).toContain("לא נמצאו פערים מהותיים")` למבחן ה-topGaps-הריק הקיים — לפני התיקון היה אפשר למחוק את שורת הטקסט החיובית והמבחן עדיין היה ירוק (הוא בדק רק היעדר "פערים מובילים").
> 14. **חוב טכני שנרשם, לא נפתר כאן:** `parseArgs` משוכפל בין `src/cli.ts` (פרסור `--pick` בלבד, פשוט) ל-`src/cli-diagnose.ts` (פרסור `--pick`+`--url` עם ולידציה). איחוד לפונקציה משותפת ב-`src/cli-shared.ts` נדחה לתוכנית 2ב — לא נדרש כדי ששני ה-CLI יתנהגו נכון היום, אבל זו כפילות אמיתית שכדאי לסגור לפני שמתווסף דגל שלישי.
>
> המקור המחייב: `src/cli-diagnose.ts`, `tests/cli-diagnose-args.test.ts`, `tests/cli-format.test.ts`.

- [x] **Step 5: ירוק** — `npx vitest run` → כל החבילה PASS (168/168, 17 קבצים — אחרי סבב הסקירה השני). `npm run typecheck` נקי.
...
> **הערה על סבב הסקירה השני:** תיקוני הפריטים 7–13 לעיל (ולידציית ארגומנטים, השלמת `website`, שימור שגיאת סריקה, אזהרת `--url`+שם, יישור קוד יציאה, שמירת שורת הפער החיובי) אומתו רק אופליין — `npx vitest run` + `npm run typecheck` — **בלי** הרצת עשן חיה נוספת (הוראה מפורשת של הסוקר: לא להריץ עוד API חי בסבב הזה). ההשלמה `prisma.business.update` על מסלול Places, שימור השגיאה ב-revert, ואזהרת `--url`+שם לא ניתנות לבדיקה אופליין (נוגעות ב-Prisma חי בתוך `main()`) — אומתו בקריאת קוד וב-typecheck בלבד; יאומתו בפועל בהרצת העשן הבאה.
