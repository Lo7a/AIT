...
// מנקזים קודם את שאלות החובה שטרם נשאלו, ואז שאלת הסיכום, בדיוק פעם אחת.
...
export function pickNextQuestion(
  ...
  // ניקוז שאלות החובה: רץ רק אחרי שהלולאה הרחבה מיצתה את עצמה (רוחב לפני עומק - סדר הראיון
  // הקיים נשמר בדיוק כפי שהיה), ולפני שאלת הסיכום. בלי הניקוז הזה השאלות האלה היו קוד מת
  // בייצור: הן אינן השאלה הראשונה בסקציה שלהן, והסקציה מזוכה כבר מהתשובה הראשונה (ראו ההערה
  // ליד REQUIRED_QUESTION_KEYS). אין סיכון ללולאה אינסופית - מוחזרת רק שאלה שאינה ב-askedKeys,
  // ו-runInterviewTurn מוסיף כל מפתח שנשאל, כך שכל תור מקטין את מספר המועמדות בדיוק באחת.
  // גם תשובה חופשית שהחילוץ שלה נכשל לא לוכדת את המשתמש: השאלה כבר נספרה כנשאלת
