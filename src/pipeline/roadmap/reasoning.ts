...
// \p{Nd} - קטגוריית יוניקוד "ספרה עשרונית" בכל סקריפט, לא רק 0-9 של ASCII - הגנה גם מפני
// ספרות שמגיעות מסקריפט אחר שה-LLM עלול להפיק. לעברית עצמה אין ספרות עשרוניות ייעודיות (המספור
// העברי המסורתי הוא אותיות/גימטריה, לא בקטגוריית Nd) - כך שההגנה בפועל שקולה ל-0-9 פשוט, אבל
// כתובה באופן כללי כדי לא להסתמך על ידיעה מוקדמת של השפה שה-LLM יבחר לכתוב בה
