...
import type { IndustrySlug } from "../src/pipeline/industry";
...
interface CatalogSeed {
  ...
  problem: string;
  solution: string;
  // industries (התניית ענף, 20.8): חסר = פריט כללי לכל עסק, וכך נשארים 18 הפריטים הקיימים.
  // conditions היא כבר עמודת Json בסכמה ולכן אין צורך במיגרציה. הסלאגים מוגדרים ב-
  // src/pipeline/industry.ts - שם מקור האמת, כולל המיפוי מסוגי Google Places
  conditions: { gapKeys: string[]; industries?: IndustrySlug[] };
  costRange: string;
  savingRange: string;
...
const CATALOG: CatalogSeed[] = [
  ...
  {
    // פריט 18 (נזרע 20.8, הכרעת מייסד): נולד מממצא חי בג'מס רעננה - שתי רשומות SPF, כלומר
    // הגנה מבוטלת. עד אז הפער היחיד בתחום היה dmarc, שהוצג בדוח בלי שום פריט שאפשר לקנות.
    // ההכרעה: ממצא אחד לשניהם, ופריט אחד שמכסה את שניהם. זה הפריט הראשון בקטלוג שהתשובה
    // הכנה עליו היא לפעמים "אל תשלמו לנו" - ולכן costRange פותח בכך שייתכן שזה כלול.
    ...
    costRange: "ייתכן שכלול אצל מי שמנהל לכם את הדומיין · אחרת עבודת טכנאי ₪250-350 לשעה · ניטור דוחות DMARC: מסלול חינמי מכסה עסק קטן",
    savingRange: "מיילים עסקיים שמגיעים לתיבה במקום לספאם, וחסימת התחזות בשם העסק - שמישהו ישלח ללקוח שלכם חשבונית מזויפת מהכתובת שלכם",
    complexity: "low",
    installTime: "עבודה קצרה של שעה-שעתיים; השינוי ב-DNS מתעדכן תוך שעות",
    benchmarks: [
      { metric: "שעת עבודת טכנאי מחשבים לעסק קטן (בנק שעות עד תמיכה מרחוק)", range: "₪250-350 לשעה", source: "9comp.com מחירון (אומת חי: בנק שעות ₪250-280, מעבדה ₪300, מרחוק ₪350)", verifiedAt: new Date("2026-08-20") },
      { metric: "קריאת שירות טכנאי (עוגן שפיות לסדר הגודל)", range: "₪200-250", source: "monitorpc.co.il מחירון (אומת חי)", verifiedAt: new Date("2026-08-20") },
      { metric: "ניטור דוחות DMARC - מסלול חינמי", range: "$0 לדומיין אחד, עד 1,000 מיילים בחודש", source: "easydmarc.com/pricing (אומת חי)", verifiedAt: new Date("2026-08-20") },
    ],
  },
  // ===== פריטים 19-20: חבילת האוכל. המחקר נגמר ב-17.8 ונחסם על התניית ענף; שוחרר 20.8 =====
  // שניהם ממופתחים על own_website ולא על online_booking, וזו הכרעה מדודה מהמחקר: הסורק מזהה
  // קישור לוולט/תן ביס, ולכן מסעדה שכל ההזמנות שלה עוברות דרך האגרגטורים **מרווחת** את
  // online_booking היום. דווקא מי שמשלם את העמלות הכי הרבה לא ייפול על הפער הזה
  {
    ...
    // savingRange ממוסמך במלואו, וזה נדיר: האחוז מגיע מחמישה פרסומים עיתונאיים 2019-2025.
    // הניסוח **חייב** לשמור על "לפי פרסומים בעיתונות" ועל טווח השנים - זו לא מדידה שלנו
    savingRange: "עמלת פלטפורמות המשלוחים ממסעדות תועדה בעיתונות ב-25-33% מכל הזמנה (2019-2025) - בערוץ ישיר העלות היא מנוי קבוע",
    complexity: "medium",
    installTime: "לא פורסם זמן הטמעה אצל הספקים - להשלים מול ספק",
    benchmarks: [
      { metric: "עמלת פלטפורמות משלוחים ממסעדות (וולט/תן ביס, לפי פרסומים)", range: "25-33% מכל הזמנה", source: "כלכליסט 12/2019 (מנהל פעילות וולט: 27%) · וואלה 3/2020 (25-33%) · ynet 4/2020 (וולט 27%; תן ביס 5-10% + כ-17% שליחות) · פואנטה 9/2022 (ייצוגית: 27%) · ישראל היום 4/2025 (מסעדנים בוועדת הכלכלה: עד 32%) - ב-2026 מדווח קרב עמלות, האחוזים העדכניים לא פורסמו (TheMarker 5/2026)", verifiedAt: new Date("2026-08-17") },
  ...
  },
  {
    ...
    conditions: { gapKeys: ["own_website"], industries: ["food_dine_in", "food_takeaway"] },
    costRange: "מנוי כ-₪57-149 לחודש + הקמה עד ₪999 · או הקמה חד-פעמית ₪1,500-2,500",
    // **בלי ספרות במכוון.** המחקר הציע נתון MGH (77% מהסועדים בודקים אתר לפני ביקור), אבל הוא
    // נושא בדיוק את שלושת הסייגים שהפילו את נתון Deloitte בסבב קודם: ארה"ב, ישן (2019), וצד
    // מעוניין. אותה פסיקה חלה כאן, והחלופה חסרת-הספרות היא זו שנזרעה
    savingRange: "סועדים בודקים תפריט לפני שהם באים - תפריט שלא נמצא בגוגל הוא לקוח שלא נכנס לשיקול",
    complexity: "low",
    installTime: "ימים בודדים (SaaS בהקמה עצמית)",
    benchmarks: [
      { metric: "תפריט דיגיטלי QR (מנוי)", range: "כ-₪57-149 לחודש + הקמה עד ₪999 (חלקו הומר מדולר לפי שער AIT הקבוע 3.0)", source: "algora.co.il/מחירון ₪149 + ₪999 (אומת חי; עמוד השיווק מציג 'החל מ-99' - סתירה שנרשמה) · stravopys.com $19 לחודש (אומת חי) (08/2026)", verifiedAt: new Date("2026-08-17") },
